...
with st.expander("🎯 نحوه محاسبه امتیازات", expanded=True):
    ...
    with col1:
        st.info(f"🏆 **تیم صعودکننده صحیح (فقط در مسابقات حذفی)**\n\n{QUALIFIED_TEAM_POINTS} امتیاز")
        
    with col2:
        st.info(f"📊 **مثال:**\n\nپیش‌بینی صعودکننده: ایران\n\nصعودکننده واقعی: ایران ✓")
...
st.info(
    ...
    - نتیجه دقیق + تیم صعودکننده = {EXACT_SCORE_POINTS} + {QUALIFIED_TEAM_POINTS} = {EXACT_SCORE_POINTS + QUALIFIED_TEAM_POINTS} امتیاز
...
st.warning(
    ...
    **شرط اساسی:**
    ...
    - ⏱️ زمان کوتاه‌شده و پنالتی در امتیازدهی دخالت ندارند
